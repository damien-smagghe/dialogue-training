...
function DecryptButton({ onClick, disabled, isLoading, children }: DecryptButtonProps) {
  return (
    <button
      onClick={onClick}
...
function ModalButton({ onClick, variant = 'primary', children, disabled, isLoading }: ModalButtonProps) {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled || isLoading}
      style={{ ...baseStyles, ...(disabled || isLoading ? disabledStyles : {}), ...hoverStyles }}
      type="button"
    >
...
function DialoguesDecrypt({ onSuccess, onError, onClose }: DecryptComponentProps) {
  ...
  useEffect(() => {
    if (showModal) {
      const input = document.getElementById('dialogue-password') as HTMLInputElement
      if (input) {
        input.focus()
      }
    }
  ...
  const handleDecrypt = useCallback(async () => {
    ...
    try {
      ...
      const dialoguesData = await crypto.decrypt(encryptedData, 'SHA-256', 100000)
      ...
      onSuccess({ dialogues: dialoguesData, characters: Array.from(characters).sort() })
    } catch (err) {
    ...
    <div className={styles.modalOverlay} role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div className={styles.modalContent}>
        ...
        <header className={styles.modalHeader}>
          <h1 id="modal-title" className={styles.modalTitle}>Authentification</h1>
        </header>
